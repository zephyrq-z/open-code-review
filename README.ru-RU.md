...
2. Если значение заканчивается на `.md` / `.txt` / `.markdown` → **путь к файлу**.
   - Абсолютные пути (начинающиеся с `/`) используются напрямую.
   - Относительные пути сначала проверяются в корне проекта; если не найдены — пробуются как есть (абсолютный путь). Если всё ещё не найдены — выводится `[WARN]`.
   - Файл должен пройти проверку: допустимое расширение, ≤ 100 KB, цель симлинка также должна иметь допустимое расширение.
...
- `docs/sql-rules.md` — относительный путь, загружается из `<project>/docs/sql-rules.md` (или как абсолютный путь, если не найден).
- `Always check for null safety…` — встроенная строка, используется напрямую.
- `shared/go-concurrency.md` — относительный путь, тот же двухуровневый поиск.
