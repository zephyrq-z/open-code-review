...
2. 如果值以 `.md` / `.txt` / `.markdown` 结尾 → **文件路径**。
   - 绝对路径（以 `/` 开头）直接使用。
   - 相对路径先在项目根目录下查找，找不到则作为绝对路径再试一次，仍找不到则视为无效（输出 `[WARN]`）。
   - 文件需通过安全校验：白名单扩展名、≤ 100 KB、symlink 解析后目标也必须是白名单扩展名。
3. 否则 → **内联内容**。
...
- `docs/sql-rules.md` — 相对路径，从 `<project>/docs/sql-rules.md` 加载（找不到则在文件系统中作为绝对路径查找）。
- `始终检查空值安全…` — 内联字符串，直接使用。
